...
# ─── PAGE CONFIG ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ECSBC 2024 Compliance Dashboard - Exceptions V4.0",
    page_icon="🏢",
    layout="wide",
...
<div class="main-header">
    <h1 style="margin:0;font-size:1.9rem">🏢 ECSBC 2024 Building Compliance Check - Exceptions V4.0</h1>
...
with st.sidebar:
    ...
    st.markdown("---")
    # st.caption("ℹ️ ECSBC 2024, Bureau of Energy Efficiency | v4.0 — full PDF exception & compliance-form coverage")
...
with tabs[1]:
    ...
    if wwr > MAX_WWR:
        ...
        # ── Whole Building Performance path modeling exceptions (§12.5) ──────────
        with st.expander("📋 Whole Building Performance Path – Modeling Exceptions (12.5)", expanded=True):
            st.markdown('<div class="exc-box">🔶 <b>12.5 Modeling Exceptions</b> — Because WWR &gt; 40%, the Whole Building Performance path is required. The following simplifications are permitted in the energy simulation model:<br><br>'
                        '<b>(a) Envelope assemblies &lt;5% of total area</b>: Need not be separately described; add their area to the adjacent assembly of the same type.<br>'
                        '<b>(b) Surfaces within ±45° orientation/tilt</b>: May be combined as a single surface or modeled with multipliers.<br>'
                        '<b>(c) Operating schedules</b>: May differ between Baseline and Proposed only where necessary to model non-standard efficiency measures (e.g., auto lighting controls, natural ventilation, DCV). Manual controls are NEVER eligible. Subject to AHJ approval.<br>'
                        '<b>(d) Identical HVAC zones</b>: Zones with similar occupancy, loads, setpoints, HVAC type, and glazed walls within ±45° orientation may be combined into a single thermal block.</div>', unsafe_allow_html=True)
            # ── NEW EXCEPTIONS §12.5.2(a)/(e)/(h): Baseline HVAC modeling rules ──
            st.markdown('<div class="exc-box">🔶 <b>Exception §12.5.2(a)</b>: Where §6.2.2 imposes specific '
                        'component-efficiency requirements, the Baseline design component shall be adjusted to the '
                        '<b>lowest efficiency level</b> allowed for that component type. Minimum outdoor-air ventilation '
                        'rates are identical for Baseline and Proposed except when modeling DCV (not required in Baseline '
                        'per §6.2.1(c)) or when the Proposed ventilation exceeds the code minimum.</div>', unsafe_allow_html=True)
            st.markdown('<div class="exc-box">🔶 <b>Exception §12.5.2(e)</b>: Airside economizers shall <b>NOT</b> be '
                        'modeled in the Baseline design for HVAC System <b>Type A</b>.</div>', unsafe_allow_html=True)
            st.markdown('<div class="exc-box">🔶 <b>Exception §12.5.2(h)</b>: Air-cooled chillers are allowed in the '
                        'Baseline design if the Proposed design has air-cooled chillers. For a mix of air- and '
                        'water-cooled chillers, the Baseline shall be modeled with the <b>same proportion</b> as the '
                        'Proposed design.</div>', unsafe_allow_html=True)
    else:
...
with tabs[3]:
    ...
    # ── NEW EXCEPTION: Exterior Lighting Power exemptions (Exception to exterior
    #    lighting power) — theatrical, monument, temporary, industrial,
    #    equipment-integral applications are excluded from the exterior power limit.
    ext_exempt_watts = 0.0
    with st.expander(f"**Exception to Exterior Lighting Power (§7.3.5)** "):
        st.markdown(
            '<div class="info-box">The following exterior lighting is <b>excluded</b> from the '
            "exterior lighting power limit and shall not be counted in the installed exterior "
            "lighting power: (a) lighting integral to equipment/instrumentation installed by its "
            "manufacturer; (b) theatrical purposes only (performance, stage, film/video "
            "production); (c) temporary lighting not permanently installed; (d) industrial "
            "activity lighting (manufacturing, material handling, transportation/storage) with "
            "hoods or louvers for glare control; (e) lighting for monuments of national "
            "importance, national flag, statues/sculptures.</div>",
            unsafe_allow_html=True,
        )
        ext_exempt_cats = st.multiselect(
            "Exterior lighting categories present that qualify for exemption",
            [
                "Equipment/instrumentation-integral (manufacturer installed)",
                "Theatrical / stage / film / video production",
                "Temporary (not permanently installed)",
                "Industrial activity (with hoods/louvers for glare control)",
                "Monument / national flag / statue / sculpture",
            ],
            key="ext_exempt_cats",
        )
        if ext_exempt_cats:
            ext_exempt_watts = st.number_input(
                "Exempt exterior lighting wattage to exclude (W) — §7.3.5 exception",
                min_value=0.0,
                value=0.0,
                step=50.0,
                key="ext_exempt_watts",
            )
            st.markdown(
                f'<div class="exc-box">🔶 <b>Exception to Exterior Lighting Power</b>: '
                f"{len(ext_exempt_cats)} exempt category(ies) selected — "
                f"<b>{ext_exempt_watts:,.0f} W</b> excluded from the exterior lighting power "
                "limit. Trade-offs between (non-exempt) applications remain not permitted.</div>",
                unsafe_allow_html=True,
            )
            light_results["7.3.5 Exterior Power Exemption"] = True
    ...
    with c1:
        ...
        ext_area = st.number_input(
            "Exterior lighted area (m²) — for exempt-wattage adjustment",
            min_value=0.0,
            value=0.0,
            step=10.0,
            key="ext_area",
        )
        ext_lpd_prop_gross = st.number_input(
            "Proposed Exterior LPD (W/m²) [before exemptions]", min_value=0.0, value=4.5, step=0.5
        )
        # Deduct exempt exterior wattage (converted to W/m²) when an area is given.
        ext_lpd_deduction = (ext_exempt_watts / ext_area) if ext_area > 0 else 0.0
        ext_lpd_prop = max(0.0, ext_lpd_prop_gross - ext_lpd_deduction)
    with c2:
        ext_pass = ext_lpd_prop <= ext_lpd_allowed
        light_results["7.3.5 Exterior LPD"] = ext_pass
        if ext_exempt_watts > 0 and ext_area > 0:
            st.metric("Exempt exterior watts excluded", f"{ext_exempt_watts:,.0f} W")
            st.metric("Effective Exterior LPD", f"{ext_lpd_prop:.2f} W/m²",
                      delta=f"Limit {ext_lpd_allowed} W/m²")
        st.markdown(
            f"**Exterior LPD:** {check_icon(ext_pass)} {ext_lpd_prop:.2f} vs max {ext_lpd_allowed} W/m²"
        )
...
with tabs[4]:
    ...
    # ── NEW CHECK: 8.2.8 Power Quality (voltage & current distortion limits) ──────
    with st.expander(f"**8.2.8 – Power Quality** "):
        st.markdown('<div class="info-box">§8.2.8: Voltage and current distortion at the point of '
                    'common coupling shall be within the limits of IEEE 519 / CEA regulations '
                    '(typically voltage THD ≤ 5%, current TDD per short-circuit ratio).</div>',
                    unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        with c1:
            v_thd = st.number_input("Voltage THD at point of common coupling (%)",
                                    min_value=0.0, max_value=50.0, value=4.0, step=0.5, key="v_thd")
            i_tdd = st.number_input("Current TDD (%)",
                                    min_value=0.0, max_value=50.0, value=6.0, step=0.5, key="i_tdd")
        with c2:
            pq_pass = v_thd <= 5.0 and i_tdd <= 8.0
            elec_results["8.2.8 Power Quality (THD/TDD)"] = pq_pass
            st.markdown(f"Voltage THD: {check_icon(v_thd<=5.0)} {v_thd}% (≤5%)  |  "
                        f"Current TDD: {check_icon(i_tdd<=8.0)} {i_tdd}% (≤8%)")

    # ── NEW CHECK: 8.2.9 Power Distribution System (cable sizing losses) ──────────
    with st.expander(f"**8.2.9 – Power Distribution System** "):
        st.markdown('<div class="info-box">§8.2.9: Cables shall be sized such that distribution '
                    'losses remain within the limits prescribed by the code.</div>',
                    unsafe_allow_html=True)
        pd1 = st.selectbox("Cables sized so distribution losses are within code limits?",
                           ["Yes","No","N/A"], key="pd1")
        pd2 = st.selectbox("Distribution loss calculations submitted?",
                           ["Yes","No","N/A"], key="pd2")
        elec_results["8.2.9 Power Distribution Losses"] = pd1=="Yes" and pd2=="Yes"

    # ── NEW CHECK: 8.2.12 Vertical Transportation (lifts/escalators/moving walks) ──
    with st.expander(f"**8.2.12 – Vertical Transportation** "):
        st.markdown('<div class="info-box">§8.2.12: Lifts, escalators and moving walks shall meet '
                    'the energy-performance requirements of §8.2.12 (e.g. regenerative drives, '
                    'standby/sleep modes, VVVF drives, energy-efficient lighting in cabins).</div>',
                    unsafe_allow_html=True)
        vt_present = st.checkbox("Building has lifts / escalators / moving walks?", key="vt_present")
        if vt_present:
            vt1 = st.selectbox("Lifts with VVVF drives and standby/sleep mode?", ["Yes","No","N/A"], key="vt1")
            vt2 = st.selectbox("Escalators/moving walks with auto start-stop or VFD?", ["Yes","No","N/A"], key="vt2")
            vt3 = st.selectbox("Energy-efficient cabin lighting & ventilation with auto-off?", ["Yes","No","N/A"], key="vt3")
            elec_results["8.2.12 Vertical Transportation"] = all(x=="Yes" for x in [vt1,vt2,vt3])
        else:
            st.markdown('<div class="info-box">No vertical transportation — §8.2.12 not applicable.</div>', unsafe_allow_html=True)
            elec_results["8.2.12 Vertical Transportation (N/A)"] = True
...
with tabs[5]:
    ...
    with c1:
        with st.expander("**9.2.1 – Source & Quality**"):
            ...
            # ── NEW EXCEPTION §9.2.1(c): reclaimed water NOT recommended for
            #    Hospital / Outpatient Health Care due to associated health hazards.
            reclaimed_used = st.checkbox(
                "Reuse of reclaimed/treated water from STP planned for non-potable use?",
                key="reclaimed_used",
            )
            if reclaimed_used and building_type == "Health Care":
                st.markdown(
                    '<div class="warn-box">🔶 <b>Exception §9.2.1(c)</b>: Reuse of reclaimed water in '
                    "<b>Hospital and Outpatient Health Care</b> is <b>NOT recommended</b> due to "
                    "associated health hazards. Reconsider the source for non-potable use.</div>",
                    unsafe_allow_html=True,
                )
                water_results["9.2.1(c) Reclaimed water (healthcare caution)"] = False
            elif reclaimed_used:
                st.markdown(
                    '<div class="exc-box">🔶 <b>§9.2.1(c)</b>: Reclaimed water permitted for non-potable '
                    "use in this occupancy (not Hospital/Outpatient Health Care).</div>",
                    unsafe_allow_html=True,
                )
                water_results["9.2.1(c) Reclaimed water permitted"] = True

        with st.expander(f"**9.2.3 – Water Treatment Systems** "):
            wt1 = st.selectbox("Representative source-water samples tested per IS 1622 / BIS 3025?", ["Yes","No","N/A"], key="wt1")
            wt2 = st.selectbox("Treatment per CPHEEO Manual on Water Supply & Treatment (2024)?", ["Yes","No","N/A"], key="wt2")
            wt3 = st.selectbox("RO/treatment design documents & test reports submitted?", ["Yes","No","N/A"], key="wt3")
            water_results["9.2.3 Water Treatment Systems"] = all(x=="Yes" for x in [wt1,wt2,wt3])
        ...
        with st.expander(f"**9.2.5 – Piping Systems** "):
            wpi1 = st.selectbox("Piping system design complies with NBC 2016?", ["Yes","No","N/A"], key="wpi1")
            water_results["9.2.5 Piping Systems (NBC 2016)"] = wpi1=="Yes"
        ...
        with st.expander(f"**9.2.7 – Controls** "):
            wc1 = st.selectbox("Water system controls (level controllers, pump controllers) per §9.2.7?", ["Yes","No","N/A"], key="wc1")
            water_results["9.2.7 Controls"] = wc1=="Yes"

        with st.expander(f"**9.2.12 – Pipe Insulation R-Value** "):
    ...
    with c2:
        with st.expander(f"**9.2.8 – Service Water Heating** "):
        ...
        # ── NEW EXCEPTIONS §9.3.5(a)/(b): Solar-PV alternative to solar water
        #    heating for Hospitality/Healthcare (Additional Mandatory).
        with st.expander(f"**9.3.5 – Service Water Heating (ECSBC+ / Super) & Solar PV Exception** "):
            st.markdown(
                '<div class="info-box">§9.3.5(a) <b>ECSBC Plus</b>: Hospitality & Healthcare shall have '
                "solar water heating for ≥60% of hot-water demand (balance 40% by other systems).<br>"
                "§9.3.5(b) <b>Super ECSBC</b>: 100% of hot-water demand via solar water heating.</div>",
                unsafe_allow_html=True,
            )
            if building_type in ["Hospitality", "Health Care"] and compliance_level in ["ECSBC+", "Super ECSBC"]:
                swh_solar_pv_alt = st.checkbox(
                    "Meeting the requirement via a Solar PV system instead of solar water heating?",
                    key="swh_solar_pv_alt",
                )
                if swh_solar_pv_alt:
                    contractual_kw = st.number_input(
                        "Total contractual power demand (kW)", min_value=0.0, value=100.0, step=10.0, key="contractual_kw"
                    )
                    pv_kw = st.number_input(
                        "Installed Solar PV capacity (kW)", min_value=0.0, value=10.0, step=1.0, key="pv_kw"
                    )
                    roof_area_pv = st.number_input(
                        "Roof/PV area (m²) for W/m² check", min_value=0.0, value=0.0, step=10.0, key="roof_area_pv"
                    )
                    if compliance_level == "ECSBC+":
                        pct_req, wpm2_req = 7.5, 300.0   # §9.3.5(a)
                        clause = "9.3.5(a)"
                    else:
                        pct_req, wpm2_req = 10.0, 400.0  # §9.3.5(b)
                        clause = "9.3.5(b)"
                    # Required PV capacity = whichever is LESS of (pct% of demand) or (W/m² × area).
                    cap_by_pct = (pct_req / 100.0) * contractual_kw
                    cap_by_area = (wpm2_req * roof_area_pv) / 1000.0 if roof_area_pv > 0 else float("inf")
                    req_pv_kw = min(cap_by_pct, cap_by_area)
                    swh_pv_ok = pv_kw >= req_pv_kw
                    if swh_pv_ok:
                        st.markdown(
                            f'<div class="exc-box">🔶 <b>Exception §{clause}</b>: Solar PV of '
                            f"{pv_kw:.1f} kW ≥ required {req_pv_kw:.1f} kW "
                            f"(lesser of {pct_req}% of contractual demand = {cap_by_pct:.1f} kW "
                            f"{'or ' + format(cap_by_area, '.1f') + ' kW by ' + str(wpm2_req) + ' W/m²' if roof_area_pv > 0 else ''}) "
                            "— solar water heating requirement is satisfied.</div>",
                            unsafe_allow_html=True,
                        )
                    else:
                        st.warning(f"§{clause}: Solar PV {pv_kw:.1f} kW < required {req_pv_kw:.1f} kW.")
                    water_results[f"{clause} Solar PV alternative to SWH"] = swh_pv_ok
                else:
                    req_pct = 60 if compliance_level == "ECSBC+" else 100
                    swh_pct2 = st.number_input(
                        f"Solar water heating % of hot-water demand (min {req_pct}%)",
                        min_value=0.0, max_value=100.0, value=float(req_pct), step=1.0, key="swh_pct2",
                    )
                    swh_add_ok = swh_pct2 >= req_pct
                    if not swh_add_ok:
                        st.warning(f"§9.3.5: requires ≥{req_pct}% solar water heating for {compliance_level}.")
                    water_results[f"9.3.5 SWH ≥ {req_pct}% ({compliance_level})"] = swh_add_ok
            else:
                st.markdown(
                    '<div class="info-box">§9.3.5 additional mandatory applies to Hospitality/Healthcare '
                    "under ECSBC+ / Super ECSBC only — not applicable for this project.</div>",
                    unsafe_allow_html=True,
                )
                water_results["9.3.5 SWH (N/A)"] = True

        with st.expander(f"**9.2.13 – Water Heating Control & Safety** "):
            whc1 = st.selectbox("Gas heaters: safe exhaust & thermostat controls provided?", ["Yes","No","N/A"], key="whc1")
            whc2 = st.selectbox("Heat pump / solar control specifications submitted?", ["Yes","No","N/A"], key="whc2")
            whc3 = st.selectbox("Energy-consumption calculations for water heating submitted?", ["Yes","No","N/A"], key="whc3")
            water_results["9.2.13 Water Heating Control & Safety"] = all(x=="Yes" for x in [whc1,whc2,whc3])

        with st.expander(f"**9.2.14 – Swimming Pool Heating** "):
            has_pool = st.checkbox("Project has a heated swimming pool?", key="has_pool")
            if has_pool:
                pool_cover = st.selectbox("Pool cover provided to reduce heat loss (§9.2.14)?", ["Yes","No","N/A"], key="pool_cover")
                water_results["9.2.14 Swimming Pool Cover"] = pool_cover=="Yes"
            else:
                st.markdown('<div class="info-box">No heated pool — §9.2.14 not applicable.</div>', unsafe_allow_html=True)
                water_results["9.2.14 Swimming Pool (N/A)"] = True

        with st.expander(f"**9.2.17 – Wastewater Treatment** "):
        ...
        with st.expander(f"**9.2.18 – Rainwater Harvesting** "):
...
with tabs[7]:
    ...
    with c2:
        ...
        with st.expander(f"**11.3.3 – Visual Comfort (ECSBC+ / Super ECSBC)** "):
            if compliance_level in ["ECSBC+", "Super ECSBC"]:
                vc3 = st.selectbox(
                    "Visual comfort conformance demonstrated for ECSBC+/Super ECSBC (uniformity, glare, illuminance)?",
                    ["Yes","No","N/A"], key="vc3",
                )
                ieq_results["11.3.3 Visual Comfort (ECSBC+/Super)"] = vc3=="Yes"
            else:
                st.markdown('<div class="info-box">§11.3.3 additional visual-comfort conformance applies to ECSBC+ / Super ECSBC only.</div>', unsafe_allow_html=True)
                ieq_results["11.3.3 Visual Comfort (N/A)"] = True

...
st.markdown("""
    <style>
        .block-container { padding-top: 1.8rem; padding-bottom: 0rem; }
    </style>
""", unsafe_allow_html=True)
...
PIPE_R_REDUCTION  = 0.2   # for conditioned-space / underground pipes
PIPE_R_INCREASE   = 0.2   # for outdoor weather-exposed pipes
PIPE_R_MIN        = 0.4   # minimum R-value after reduction
PIPE_R_ALT_PERF   = 85.0  # % performance improvement for alternate material (Table 9.2 note)
PIPE_R_ALT_9_12   = 88.0  # % for Table 9.12
PIPE_R_ALT_9_13   = 92.0  # % for Table 9.13
...
with st.sidebar:
    ...
    # ── NEW EXCEPTION 1: Project Type (§3.3.2) ────────────────────────────────
    st.markdown("---")
    ...
    # Renewable Energy flag (shared across tabs for §9.3.5(b) exception)
    st.markdown("---")
    ...
    re_type_sidebar = st.multiselect(
        ...
        default=[],
        help="Used across tabs to trigger §9.3.5(b) sanitary ware exception and §8.2.11 checks."
    )
    ...
    if is_mixed_use:
        st.markdown('<div class="warn-box">Enter sub-use areas below. If a sub-use is &lt;10% of AGA, the dominant type rules.</div>', unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.caption("ℹ️ ECSBC 2024, Bureau of Energy Efficiency | v2.0 — All exceptions implemented")
...
with tabs[0]:
    ...
    # §3.3.2 banner for alterations
    if project_type == "Addition or Alteration to Existing Building":
        st.markdown('<div class="exc-box">🔶 <b>§3.3.2 Active</b>: This is an Alteration project. Site provisions tied to existing infrastructure may be exempt. Only new site works must comply.</div>', unsafe_allow_html=True)
    ...
    with st.expander("**4.2.1 – Topsoil Preservation**", expanded=True):
        ...
        with c1:
            ...
            ts2 = st.selectbox("Calculations of topsoil quantity preserved and used?",      ["Yes","No","N/A"], key="ts2")
            ts3 = st.selectbox("Site plan (DWG) highlighting excavation/preservation areas?",["Yes","No","N/A"], key="ts3")
            ts4 = st.selectbox("Date-stamped photographs of measures taken?",               ["Yes","No","N/A"], key="ts4")
        with c2:
    ...
    with st.expander("**4.2.2 – Tree Preservation and Planting**"):
        ...
        with c1:
            ...
            tr3 = st.selectbox("Purchase orders for new plantation materials?",  ["Yes","No","N/A"], key="tr3")
            tr4 = st.selectbox("Calculations of trees planted and preserved?",  ["Yes","No","N/A"], key="tr4")
        with c2:
    ...
    with st.expander("**4.2.4 & 4.3.3 – Design for Differently Abled**"):
        c1, c2 = st.columns([2, 1])
        with c1:
            da1 = st.selectbox("Ramps, elevator & washroom designed per code requirements?", ["Yes","No","N/A"], key="da1")
        with c2:
            p = da1=="Yes"
    ...
    with st.expander("**4.2.6 – Brownfield Remediation**"):
        c1, c2 = st.columns([2, 1])
        with c1:
            br1 = st.selectbox("Brownfield remediation techniques per local bylaws?", ["Yes","No","N/A"], key="br1")
            br2 = st.selectbox("Approval of local statutory body for intended use?",  ["Yes","No","N/A"], key="br2")
        with c2:
    ...
    with st.expander("**4.3.8 – Heat Island Reduction (Roof)**"):
        c1, c2 = st.columns([2, 1])
        with c1:
            cr1 = st.selectbox("Exposed roof vs vegetated/cool roof documented?", ["Yes","No","N/A"], key="cr1")
...
with tabs[1]:
    ...
    st.markdown(f'##### SHGC Input Method <span class="new-badge">NEW §5.2.1(b)</span>', unsafe_allow_html=True)
    ...
    # VLT rating check
    st.markdown(f'##### VLT Rating Check <span class="new-badge">NEW §5.2.1(c)</span>', unsafe_allow_html=True)
...
with tabs[2]:
    ...
    with st.expander(f"**6.2.1 – Ventilation** {new_badge()}", expanded=True):
...
with tabs[3]:
    ...
    with c1:
        ...
        with st.expander(f"**7.2.2(b) – Space Control** {new_badge()}"):
            ...
            # ── NEW EXCEPTION 9: Remote control device §7.2.2(b)-V ───────────
            ...
            if remote_ctrl_needed:
                ...
                remote_exc_pass = pilot_light_ok and labelled_ok
                if remote_exc_pass:
                    st.markdown('<div class="exc-box">🔶 <b>Exception §7.2.2(b)-V</b>: Remote control device permitted — pilot light indicator and labelling confirmed. Location constraint is WAIVED.</div>', unsafe_allow_html=True)
                else:
                    missing = []
    ...
    exempt_wattage = st.number_input("Exempt lighting wattage to be excluded (W) (§7.3 — display, medical, food-warming, etc.)",
        min_value=0.0, value=0.0, step=50.0,
        help="Only counts if these have independent controls and are additive to general lighting.")
    ...
    if multi_lighting_systems:
        highest_system_watts = st.number_input(
            ...
            help="Only this system's wattage is used in the LPD calculation per §7.3.3."
        )
        st.markdown('<div class="exc-box">🔶 <b>Exception §7.3.3</b>: Multiple independent non-simultaneous systems — LPD calculated using only the highest-wattage system. Lighting quality and quantity must not be compromised.</div>', unsafe_allow_html=True)
    ...
    with c1:
        ...
        if multi_lighting_systems and highest_system_watts > 0:
            # §7.3.3: use only the highest system, minus other exemptions
...
with tabs[4]:
    ...
    st.markdown('<div class="info-box">Note: RE type is also set in the sidebar and shared across tabs (e.g., for §9.3.5(b) sanitary ware exception).</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        ...
        st.markdown(f"**RE Systems:** {check_icon(re_ok)} {', '.join(re_type_sidebar) if re_ok else 'None'}  |  **REGZ ≥ 50%:** {check_icon(regz_ok)} {regz_pct:.0f}%")
...
with tabs[5]:
    ...
    with c1:
        ...
        # ── NEW EXCEPTION 12: Pipe insulation R-value §9.2.12 ────────────────
        with st.expander(f"**9.2.12 – Pipe Insulation R-Value** {new_badge()}"):
            st.markdown('<div class="info-box">§9.2.12: Required insulation R-value varies based on pipe location. Reduction or increase from Table 9.2 values applies based on exposure.</div>', unsafe_allow_html=True)
            ...
            use_alt_insulation = st.checkbox(
                "Using alternate insulation material instead of Table 9.2 R-values?",
                help="§9.2.12 / Table 9.2 Exception: Alternate material must improve performance by 85% vs reference."
            )

            if use_alt_insulation:
                alt_level = st.selectbox(
                    "Which table is being replaced?",
                ...
                alt_perf = st.number_input(f"Demonstrated performance improvement of alternate material (%)", min_value=0.0, max_value=100.0, value=req_perf, step=0.5)
                alt_ok = alt_perf >= req_perf
                if alt_ok:
                    st.markdown(f'<div class="exc-box">🔶 <b>Alternate Insulation Exception ({alt_level})</b>: Performance improvement = {alt_perf}% ≥ required {req_perf}% → <b>PASS</b>. Table R-values need not be used.</div>', unsafe_allow_html=True)
                else:
                    st.warning(f"Alternate insulation: {alt_perf}% < required {req_perf}% — does not qualify for exception.")
                ...
                if pipe_location == "In conditioned-space partition or underground (R-value may be reduced by 0.2, min R-0.4)":
                    effective_r = max(PIPE_R_MIN, table_r - PIPE_R_REDUCTION)
                    st.markdown(f'<div class="exc-box">🔶 <b>§9.2.12 Exception — Reduced R</b>: Table 9.2 R = {table_r} → Effective required R = max(R-0.4, {table_r} − 0.2) = <b>{effective_r:.2f} m²·K/W</b></div>', unsafe_allow_html=True)
                elif pipe_location == "Outside building with direct weather exposure (R-value must be increased by 0.2)":
                    effective_r = table_r + PIPE_R_INCREASE
                    st.markdown(f'<div class="exc-box">🔶 <b>§9.2.12 — Increased R (weather exposed)</b>: Table 9.2 R = {table_r} + 0.2 = <b>{effective_r:.2f} m²·K/W</b> required</div>', unsafe_allow_html=True)
                else:
                    effective_r = table_r
                    st.markdown(f'Standard location — Required R = <b>{effective_r:.2f} m²·K/W</b> (Table 9.2 value used as-is)')
    ...
    with c2:
        # ── NEW EXCEPTION 12b: Solar water heating / condenser heat recovery ──
        with st.expander(f"**9.2.8 – Service Water Heating** {new_badge()}"):
            ...
            if condenser_recovery:
                st.markdown('<div class="exc-box">🔶 <b>Exception §9.2.8</b>: Condenser heat recovery (from chillers) is an accepted alternate to solar water heating for providing hot water. This satisfies the §9.2.8 requirement.</div>', unsafe_allow_html=True)
            # Hospitality / Healthcare 40% solar minimum
            if building_type in ["Hospitality","Health Care"] and not condenser_recovery:
                hosp_solar_pct = st.number_input("Solar water heating % of total hot water demand (Hospitality/Healthcare min 40%)", min_value=0.0, max_value=100.0, value=40.0, step=1.0)
                swh_hosp_ok = hosp_solar_pct >= 40.0
                if not swh_hosp_ok:
                    st.warning(f"§9.2.8: Hospitality/Healthcare requires ≥40% solar water heating. Remaining may be met by High Energy Efficient System.")
                water_results["9.2.8 Hospitality/Healthcare SWH ≥ 40%"] = swh_hosp_ok
            water_results["9.2.8 Service Water Heating"] = len(swh_type) > 0

        # ── NEW EXCEPTION 11: Wastewater threshold §9.2.2(c) ─────────────────
        with st.expander(f"**9.2.17 – Wastewater Treatment** {new_badge()}"):
            st.markdown(f'<div class="info-box">§9.2.2(c) Exception: Wastewater treatment is NOT mandatory if generation is &lt;10 kL/day.</div>', unsafe_allow_html=True)
            ww_gen = st.number_input(
                "Estimated wastewater generation (kL/day)",
                min_value=0.0, value=15.0, step=0.5,
                help="§9.2.2(c): If <10 kL/day, STP/reclamation system is not mandatory."
            )
            if ww_gen < 10.0:
                st.markdown(f'<div class="exc-box">🔶 <b>Exception §9.2.2(c)</b>: Wastewater generation = {ww_gen:.1f} kL/day &lt; 10 kL/day — STP/reclamation system is <b>NOT mandatory</b>.</div>', unsafe_allow_html=True)
        ...
        with st.expander("**9.2.16 – Water Efficiency**"):
            # ── NEW EXCEPTION 15: Solar PV cross-reference for §9.3.5(b) ─────
            if solar_pv_installed:
                st.markdown(f'<div class="exc-box">🔶 <b>Exception §9.3.5(b)</b>: Solar PV is installed (confirmed in sidebar) — relaxed sanitary ware flow rate requirements apply under IS 17650. Standard stricter flow rate thresholds are waived for ECSBC+ / Super ECSBC sanitary ware.</div>', unsafe_allow_html=True)
            we1 = st.selectbox(
        ...
        # ── NEW EXCEPTION 14: RWH seasonal period exception §9.2.15 ─────────
        with st.expander(f"**9.2.18 – Rainwater Harvesting** {new_badge()}"):
            ...
            prolonged_rainfall = st.checkbox(
                ...
                help="§9.2.15 Exception: Where rainfall is spread over a prolonged period, dry/wet seasons may be defined per actual recorded rainfall with Meteorological Department documentation."
            )
            if prolonged_rainfall:
                met_doc = st.checkbox("Documented evidence from Meteorological Department available?")
                if met_doc:
                    st.markdown('<div class="exc-box">🔶 <b>Exception §9.2.15</b>: Prolonged rainfall pattern confirmed with Meteorological Department documentation — dry season / wet season periods may be defined per actual seasonal rainfall recorded, deviating from standard seasonal definitions.</div>', unsafe_allow_html=True)
                    water_results["9.2.15 RWH Seasonal Period Exception"] = True
                else:
                    st.warning("§9.2.15 Exception requires Meteorological Department documentation to be provided.")
...
with tabs[6]:
    ...
    with st.expander("**Post-Construction Organic Waste (§10 area threshold)**"):
        ...
        with c1:
            if gross_area < 5000:
                st.markdown(f'<div class="exc-box">🔶 BUA {gross_area:,.0f} m² &lt;5,000 m²: Project may hand organic waste to the local body if municipal pick-up is available. On-site composter required only if no municipal arrangement exists.</div>', unsafe_allow_html=True)
                pw1 = st.selectbox("Municipal organic waste pick-up arrangement OR on-site composter provided?", ["Yes","No","N/A"], key="pw1")
            else:
                st.markdown(f'<div class="info-box">BUA ≥5,000 m²: On-site composting of ≥50% of projected organic waste is mandatory.</div>', unsafe_allow_html=True)
...
with tabs[8]:
    ...
    with c2:
        st.markdown("**All exceptions implemented (v2.0):**")
        st.markdown("""
**Original exceptions:**
- 🔶 5.3.2 Wall U relaxed (uncond. No-Star Hotel / Healthcare / School, non-Cold)
- 🔶 5.3.3 Exc.1 SEF-based equivalent SHGC (Tables 5.12 & 5.13)
- 🔶 5.3.3 Exc.2 High-sill fenestration SHGC exemption (TEA + light-shelf PF)
- 🔶 5.3.3 Exc.3 Surrounding obstructer shading (80% summer solstice)
- 🔶 5.3.3 Unconditioned fenestration U up to 5.0 W/m²·K
- 🔶 5.3.4 Skylight exception for unconditioned/temporary spaces
- 🔶 5.3.5 Full EPF Trade-off (Tables 5.16–5.20)
- 🔶 5.2.3 Daylighting targets by type; Assembly exempted
- 🔶 5.3.3 North SHGC latitude split (≥15°N vs <15°N)
- 🔶 6.2.3(a) Timeclock exempt <17.5 kWr
- 🔶 6.2.3(d) Cooling tower control only for BUA >20,000 m²
- 🔶 6.2.3(e) AHU fan VSD exempt <5,000 m³/hr
- 🔶 6.2.3(f) Damper exempt for kitchen exhaust hoods
- 🔶 6.3.1 Fan FEI exempt for un-ducted AC units
- 🔶 6.3.11 Energy recovery exempt: kitchen/laundry/OR/ICU/lab
- 🔶 7.1 Emergency/life-safety lighting excluded from LPD
- 🔶 7.2.2(a) Auto shutoff exempt: 24/7, patient care, safety
- 🔶 7.2.3 Exterior lighting exempt for emergency/firefighting
- 🔶 7.3 Exempt lighting categories excluded from LPD
- 🔶 8.2.5 DG star rating mandatory only for BUA >20,000 m²
- 🔶 10 Organic waste: <5,000 m² BUA may use local body
- 🔶 Mixed-use 10% AGA rule

**NEW exceptions (v2.0):**
- 🆕 3.3.2 Additions/Alterations: existing systems exempt
- 🆕 5.2.1(b) SHGC alternate methods: SC×0.86, glass-only SHGC
- 🆕 5.2.1(c) VLT 10% derating for unrated products
- 🆕 5.3.2 Fixed uncond. wall check (Health Care parent + subtypes)
- 🆕 6.2.1(c) Ventilation: process exhaust + energy recovery sub-exceptions
- 🆕 6.2.3(a) Single-zone HVAC VAV exception
- 🆕 6.3.5(a) Cooling tower Table 6.16 alternate compliance path
- 🆕 6.3.5(d) Economizer factory commissioning exception (AHJ certified)
- 🆕 7.2.2(b)-V Remote control device exception (pilot light + label)
- 🆕 7.3.3 Multiple non-simultaneous lighting systems LPD
- 🆕 9.2.2(c) Wastewater <10 kL/day → STP not mandatory
- 🆕 9.2.8 Condenser heat recovery as SWH alternate
- 🆕 9.2.12 Pipe insulation R±0.2 by location + alternate material
- 🆕 9.2.15 RWH seasonal period (Meteorological Dept. evidence)
- 🆕 9.3.5(b) Solar PV cross-ref for sanitary ware exception
- 🆕 12.5 Simulation modeling exceptions (envelope, schedule, HVAC zones)
        """)
        st.markdown(f"**Score:** {overall_pct:.0f}% | **Level:** {compliance_level} | **v2.0 — 38 exceptions**")